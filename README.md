...
| **5.1** | [インターフェース](./docs/05_oop.md) | クラスへのインターフェスの実装 | ✅ |
...
3. 学習と演習
  - `docs/` フォルダにある解説を読みます。各章にある課題に取り組んでください。
    - （各章へは目次のリンクから移動できます。ステータスが✅のところは実施してOKです）
  - `src/` フォルダにあるサンプルコードをローカル環境で実行して動作を確認します。（前提：ローカルPCに開発環境を構築済みであること）
  - `exercises/` 各章の課題を解き、自分のコードを Push してください。（Codespaceで実装OK）
